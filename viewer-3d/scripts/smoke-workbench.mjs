...
try {
  ...
  for (const tool of ['bedrock', 'avatar']) {
    ...
    if (tool === 'bedrock') {
      await page.waitForFunction(() => window.__ASPERGILLUM_CAPTURE__?.ready === true);
    } else {
      await page.waitForFunction(() => window.__ASPERGILLUM_AVATAR_CAPTURE__?.ready === true);
    }
  ...
  await rapidPage.waitForFunction(() => (
    ...
    && window.__ASPERGILLUM_AVATAR_CAPTURE__?.ready === true
  ));
  ...
  for (const standalone of [
    ...
    { path: 'bedrock-renderer.html', api: '__ASPERGILLUM_CAPTURE__' },
    { path: 'avatar-lab.html', api: '__ASPERGILLUM_AVATAR_CAPTURE__' },
  ]) {
  ...
  process.stdout.write(`${JSON.stringify({ firstPaint, timings, workbench, responsive }, null, 2)}\n`);
} finally {
  await browser?.close();
  await server.close();
}
