...
export const AVATAR_CAPTURE_VIEWS = Object.freeze([
  ...
  Object.freeze({
    id: 'equipment-front',
    label: 'Detalhe · equipamento · frente',
    direction: [0, 0.08, 1],
    up: [0, 1, 0],
    focus: 'equipment',
  }),
  Object.freeze({
    id: 'equipment-back',
    label: 'Detalhe · equipamento · trás',
    direction: [0, 0.08, -1],
    up: [0, 1, 0],
    focus: 'equipment',
  }),
...
export function normalizeAvatarRecipe(recipe = {}) {
  ...
  return {
    ...
    equipment: {
      project: recipe.project ?? 'aspergillum',
      id: recipe.equipmentId ?? recipe.item ?? 'aspergillum:aspergillum',
      item: recipe.item ?? recipe.equipmentId ?? 'aspergillum:aspergillum',
      slot: recipe.slot ?? 'slot.weapon.mainhand',
      targetBone: recipe.targetBone ?? 'rightItem',
      binding: recipe.binding ?? 'q.item_slot_to_bone_name(context.item_slot)',
      profile: recipe.profile ?? 'aspergillum-held-v1',
    },
    presentation: {
      perspective,
      action,
