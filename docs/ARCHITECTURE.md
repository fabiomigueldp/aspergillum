...
Timeline autoritativa de carregamento (`0.80 s`): movimento vanilla como base, correção aditiva exclusiva de `rightarm`, antecipação curta, avanço/descida, imersão, commit no tick 10, retenção e settle. A contribuição usa peso `0.32` em primeira pessoa e `1.0` em terceira; `rightitem` nunca é animado. O contêiner visual permanece até `1.10 s` somente para executar uma compensação TP dirigida por `attack_time`; depois de `0.80 s`, todos os canais artísticos já estão neutros.
