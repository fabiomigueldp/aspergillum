...
O player e o attachable mantêm pose stacks separados. O primeiro reproduz holding, swing vanilla, viewmodel FP, carga e ponte de recuperação; o segundo aplica hold FP/TP e a ação local carregada dos JSONs do pack. O adaptador central converte canais Bedrock para `ZYX` e para os perfis de câmera TP/FP. Um compositor de binding resolve a costura entre o espaço de modelo nativo e os dois graphs Three.js, conserva uma parcela grip-safe do offset TP e nunca grava compensação no `aspergillum_bound` do pack. Tanto a UI quanto `capture-avatars.mjs` consomem a mesma cena. O manifest headless inclui hashes, matrizes, erro da costura e testes OBB de mão–cabo/cabeça, permitindo repetir uma revisão sem promover os PNGs a assets.
