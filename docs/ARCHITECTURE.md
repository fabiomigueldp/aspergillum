...
A entrada humana canônica é `index.html`, o **3D Workbench**. Ela mantém um único documento e um shell persistente com navegação, contexto do runtime e ações da ferramenta ativa. A rota `?tool=model|bedrock|avatar` usa History API, portanto troca de laboratório, voltar/avançar e links profundos não recarregam a página. O shell carrega o HTML de cada laboratório como um adaptador declarativo, inicializa seu módulo real uma única vez e estaciona sua árvore DOM quando ela fica inativa. Isso preserva seleção, câmera, pose, material e demais controles sem duplicar a implementação dos laboratórios.
...
As entradas `model-lab.html`, `bedrock-renderer.html` e `avatar-lab.html` permanecem executáveis de forma autônoma. Elas são a fronteira compatível para Playwright, agentes, captura headless e diagnóstico isolado; não contêm uma segunda engine. Os CLIs continuam abrindo essas páginas diretamente e consumindo `window.__ASPERGILLUM_CAPTURE__` ou `window.__ASPERGILLUM_AVATAR_CAPTURE__`. Render loops e handlers globais verificam se seu canvas está conectado, suspendendo trabalho da ferramenta estacionada sem destruir seu estado. O smoke test do Workbench valida first paint escuro, navegação em um só documento, estado preservado, isolamento de CSS e prontidão dessas APIs autônomas.
...
O player e o attachable mantêm pose stacks separados. O primeiro reproduz holding, swing vanilla, viewmodel FP, carga e ponte de recuperação; o segundo aplica hold FP/TP e a ação local carregada dos JSONs do pack. O adaptador central converte canais Bedrock para `ZYX` e para os perfis de câmera TP/FP. Um compositor de binding resolve a costura entre o espaço de modelo nativo e os dois graphs Three.js: ele absorve integralmente a translação de hold já avaliada no graph independente para sobrepor o pivô autorado do grip ao `rightItem`, sem gravar compensação no `aspergillum_bound` do pack. Tanto a UI quanto `capture-avatars.mjs` consomem a mesma cena. O manifest headless inclui hashes, matrizes, offset XYZ do centro do cabo, testes de contenção mão–grip e colisões OBB da cabeça, permitindo repetir uma revisão sem promover os PNGs a assets.
