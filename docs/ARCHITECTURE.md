...
Itens customizados não possuem um componente estável de geometria arbitrária para a GUI: `minecraft:icon` continua raster, e attachables governam somente a apresentação segurada/equipada. A 1.2.4 usa a exceção estável documentada de `minecraft:block_placer`, que pode renderizar como ícone o bloco referenciado, combinada ao componente de bloco `minecraft:item_visual`. Um único proxy interno `aspergillum:inventory_visual` expõe `aspergillum:inventory_cosmetic ∈ 0..15`; cada item conserva seu ID e aponta para o state correspondente, sem `replace_block_item` e sem `minecraft:icon`.
