...
`assets-src/models/aspersorium.model.json` e `assets-src/models/sacristan_table.model.json` são fontes humanas dos blocos estáticos. Durante a geração, os onze cubos de `handle`/`sprinkler_head` e suas ilhas já empacotadas são transladados para o bone `resting_aspergillum`; color, normal e MER da variante são copiados para uma região reservada dos atlas `256 × 256`. Assim, nenhum estado acomodado possui uma segunda malha artística independente.
