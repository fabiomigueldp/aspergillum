...
## Arquitetura híbrida da v1.0.16
...
O bridge contém quatro partículas pequenas, vive somente `0.12–0.18 s` e avança ao longo da orientação local da cabeça. Sua função é tornar contínua a leitura ponta→leque, não acrescentar alcance ou densidade perceptível à aspersão.
...
- billboard `0.034 × 0.082` bloco antes do multiplicador por perfil;
- eixo longo `direction_y`, derivado da velocidade com threshold `0.01`;
- escala pequena no nascimento, máxima moderada no meio e reduzida no fade;
