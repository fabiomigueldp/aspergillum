...
Esta revisão preserva binding, grip, envelope físico, poses, animações, steering, partículas, áudio, docking parcial e a cabeça simples da v1.1.6. Na caldeirinha, estrutura e aspersório acomodado pertencem a um bloco integralmente `opaque`; somente a água usa `entity_alphablend` numa entidade visual mínima e autorreparável, conforme a 1.1.9c aprovada em jogo.

- Artefato: `dist/releases/Aspergillum-1.1.9.mcaddon`.
...
3. Importar somente `Aspergillum-1.1.9.mcaddon`.
4. Duplicar um mundo persistente que já tenha usado a 1.1.7; nunca executar primeiro o gate sobre o original.
...
| --- | --- |
| Encher | água aparece como projeção translúcida única, balde/política Creative corretos, mensagem localizada única |
...
| Órbita visual | item e composição acomodada correspondem; água `entity_alphablend` permanece translúcida, bloco integralmente opaco e os degraus superiores/inferiores permanecem contínuos até junto ao plano próximo |
...
| Quebrar/reload | snapshot não duplica nem desaparece; projeção é removida/recriada sem órfão e sem alterar `water_base + water_offset` |
...
| Compatibilidade | mundo 1.1.7 conserva blocos, states e snapshots; cada caldeirinha não vazia recebe uma única projeção em até seis segundos |

## Desempenho

Não criar LOD por intuição. Registrar `/script profiler start` e `/script profiler stop` com 1, 16, 64 e 256 caldeirinhas não vazias carregadas, além do gate recorrente com 1, 4, 8 e 16 jogadores executando aspersões. Anotar plataforma, gráficos, pico, média, memória e contagem de entidades. Somente uma regressão reproduzível autoriza otimização, e ela deve preservar estado, timing, aparência e autorreparo.
...
- Content Log sem mensagem atribuível ao add-on; as mensagens antigas de métodos mistos em `MaterialInstances` não são mais aceitas na arquitetura 1.1.9;
